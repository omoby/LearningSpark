...
object WordCount {
  def main(args:Array[String]): Unit ={
    ...
    conf.setMaster("local[4]") //此时在本地运行，不需要安装spark集群

    /**
      * 第二步：创建SparkContext对象，
      * SparkContext是spark程序的所有功能的唯一入口，无论是是采用scala、
      * Java，Python、R都必须有一个SparkCont
    ...
    /**
      ...
      *
      */
    val lines = sc.textFile("hdfs://localhost:9000/spark/input",2) //path:数据路径 minPartition：文件分片（并行度）
    ...
    /**
      ...
      */
    val words = lines.flatMap(line=>line.split(" ")) //对每一行的字符串进行单词拆分，并且把所有行的拆分结果通过flatMap合并唯一大集合
    ...
    /**
      ...
      */
    val wordCountsOrder= pairs.reduceByKey(_+_).map(pair=>(pair._2,pair._1)).sortByKey(false).map(pair=>(pair._2,pair._1)) //对相同的key进行value的累加（包括local和Reducer级别同时Reduce）
    wordCountsOrder.collect().foreach(wordNumberPair=>println(wordNumberPair._1 + " : "+wordNumberPair._2))

    /*wordCounts.sortBy(_._2,false).collect().foreach(wordNumberPair=>println(wordNumberPair._1 + " : "+wordNumberPair._2)) //tuple index从1开始,根据value排序
    wordCounts.sortByKey().collect().foreach(wordNumberPair=>println(wordNumberPair._1 + " : "+wordNumberPair._2)) //tuple index从1开始，根据可以排序
*/
    sc.stop()
  }
